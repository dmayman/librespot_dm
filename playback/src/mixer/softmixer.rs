...
impl Mixer for SoftMixer {
    ...
    fn set_volume(&self, volume: u16) {
        if self.config.ignore_volume {
            println!("Ignoring volume change to {}", volume);
            log::info!("Ignoring volume change to {}", volume);
            // Keep the volume at full scale
            self.volume.store(f64::to_bits(1.0), Ordering::Relaxed);
        } else {
            let mapped_volume = self.volume_ctrl.to_mapped(volume);
            self.volume.store(mapped_volume.to_bits(), Ordering::Relaxed);
        }
    }
